  ...
  const { message } = App.useApp();
        ...
        { title: '账号状态', dataIndex: 'status', width: 100, render: (s) => <StatusTag status={s === '启用' ? '已完成' : '停用'}>{s}</StatusTag> },
        {
            ...
            render: (_, record) => (
                <Space size="small" wrap>
                    ...
                    <Popconfirm
                        title="确认重置密码？"
                        description="将此账号密码重置为默认密码，此操作不可撤销。"
    ...
    return (
            <MainLayout>
                ...
                <PageHeader title="用户管理" description="管理企业账号、角色及访问状态。" breadcrumbs={[{ title: '系统管理' }, { title: '用户管理' }]} />
                <FilterPanel>
                    <QueryFilter
                        submitter={{
                ...
                <Card className="ui-table-card">
                    <TableToolbar title="用户列表" count={mockData.length} actions={
                        <SpecMarker
                          id="accounts-rbac"
                          number={1}
                ...
                <AppModal title="添加成员" open={createOpen} onCancel={() => setCreateOpen(false)} onOk={() => { setCreateOpen(false); message.success('创建成功'); }} okText="确定" cancelText="取消" widthSize="small">
                ...
                <AppModal title="编辑账号" open={editOpen} onCancel={() => setEditOpen(false)} onOk={() => { setEditOpen(false); message.success('编辑成功'); }} okText="确定" cancelText="取消">
                ...
                <AppModal title="修改密码" open={changeOpen} onCancel={() => setChangeOpen(false)} onOk={() => { setChangeOpen(false); message.success('密码修改成功'); }} okText="确定" cancelText="取消">
                    <Form layout="vertical" style={{ marginTop: 16 }}>
